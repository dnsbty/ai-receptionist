...
RUN apt-get update -y && apt-get install -y --no-install-recommends build-essential git ca-certificates curl gnupg \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/receptionist ./
